...
namespace MicrOrm.Test
{
    public class PerformanceTest
    {
        ...
        public void TestLargeQuery()
        {
            ...
            Console.WriteLine($"{result.Count} records, {sw.ElapsedMilliseconds}ms");
        ...
        protected VehicleHistoryReportModel ModelFromRecord(IDictionary<string, object> record)
        {
            var model = new VehicleHistoryReportModel()
            {
                Id = (int) record["Id"],
                AssetId = (int) record["AssetId"],
                ContainerAssetId = new List<int> {(int) record["ContainerAssetId"]},
                EventTime = new DateTime(((DateTime)record["EventTime"]).Ticks, DateTimeKind.Utc),
                ReceivedTime = new DateTime(((DateTime)record["ReceivedTime"]).Ticks, DateTimeKind.Utc),
                Longitude = (double)record["PositionLon"],
                Latitude = (double)record["PositionLat"],
                PositionError = (int)record["PositionError"],
                EventTypeName = (string) record["EventTypeName"]
            };
            return model;
        }
